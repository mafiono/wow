<template>
  <modal :opened="(nyFloridaSweepRulesReminderNeedAccepting && $route.meta.requiresAuth === 'yes' && $route.name === 'Lobby')" name="sweep-modal" ref="modal"
    :closeButton="false">
    <div class="sweep-modal">
      <div class="sweep-modal-inner">
        <div class="content">
          <h5>🌟 Attention Players from New York and Florida! 🌟</h5>
          <p>Please be advised that the maximum win from a single spin is capped at <span class="nowrap">🎰 SC 5,000 🎰</span> for players residing in <strong>New York</strong> and <strong>Florida</strong>.</p>
          <p>For more details and to stay informed, we encourage you to read our Sweepstakes Rules.</p>
          <p><router-link :to="{name: 'Sweepstakes Rules'}"><strong>👉 Click Here to View Sweepstakes Rules 👈</strong></router-link></p>
          <p>Play responsibly and good luck! 🍀</p>
        </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['playerData', 'nyFloridaSweepRulesReminderNeedAccepting']),
  },
  methods: {
    async acceptRules() {
      this.loading = true;
      await this.$store.dispatch('updateExtraInfo', { nyFloridaReminderAccepted: true });
      this.loading = false;
    },
    submitForm() {
      this.submitted = true;
